...
Identifying the optimized combinations of channels, gradients and thresholds to filter image are the crucial and hardest part of this project. To simplify this work, I created `Moudle` classes and `ImageList` (in `line_finder/thresholding.py`)  to reduce the amount of boilerplate code and help with visualization. Further, to fine tune the thresholds, I created GUI with sliding bars (in `line_finder/module_gui.py`). I also refactor the whole`workflow` a lot so that the thresholding part can be plugged as a single function into the pipeline instead of tight coupled with other part of the pipeline.
...
1. Thresholding, even though optimized carefully, is not good enough. A optimized thresholding to identify lane lines in all frames of a video might simply not exist. Indeed, I could not find one for the `harder_challenge_video`. More advanced image processing techniques are needed, which is also needed to address the next issue.
2. Not all the information is considered. For example, lane lines are usually strips of a constant width, consistent pixel values in y direction, opposite gradients on their edges in most case. This information can help with excluding a lot of false positives. Also, In some extreme road and lightning condition, the lane line might be impossible to identified, extrapolating them from previous and later frames are essential. My tricks covers a little about this point, but does not fully utilized the information in the video streams.
